...
class TestSearchResultUtils:
    ...
    def test_merge_results_with_duplicates(self) -> None:
        ...
        mock_result1 = MagicMock(spec=["content", "model", "provider", "usage"])
        mock_result1.id = 1
        mock_result2 = MagicMock(spec=["content", "model", "provider", "usage"])
        mock_result2.id = 2
        mock_result3 = MagicMock(spec=["content", "model", "provider", "usage"])
        mock_result3.id = 1  # Duplicate
        mock_result4 = MagicMock(spec=["content", "model", "provider", "usage"])
        mock_result4.id = 3
