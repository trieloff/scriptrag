...
class ScriptRAGMCPServer:
    ...
    async def _tool_get_character_relationships(
        ...
        from .database.connection import DatabaseConnection
        from .database.graph import GraphDatabase, GraphNode
        ...
        for row in rows:
            ...
            node = graph_db.get_node(node_id)  # type: ignore[assignment]
            if node:
        ...
        if character_name and target_character_node:
            ...
            if character_node_ids:
                ...
                # Import GraphEdge to properly parse edges
                from .database.graph import GraphEdge
        ...
        for _rel_key, rel_data in relationship_map.items():
            ...
            # Skip if filtering by character and this relationship doesn't involve them
            if character_name and character_name not in rel_data["characters"]:
                continue

            # Calculate interaction strength (based on dialogue count and scene count)
            ...
            relationships.append(
                {
                    ...
                    # Limit scenes for response size
                    "scenes": list(rel_data["scenes"])[:10],
                }
            )
